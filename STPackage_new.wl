...
(* ::Code:: *)
(*(* Labels for one-qubit Schur basis states *)*)
(*SchurBasis[1]={{{0,1},{1},{},{0}},{{0,1},{1},{},{1}}};*)
(*(* Labels for n-qubit Schur basis states *)*)
...
(* Extend a given list of Schur basis labels from n-1 to n qubit *)
...
SchurTransform[n_]:=Module[{SchurTrans,sb,Id=SparseArray@IdentityMatrix[2]},
	...
	SchurTrans=Id;
	(* Build up the Schur transform recursively *)
	Do[SchurTrans=PermutationMatrix[Ordering[sb=ExtendSchurBasis[Sort@sb,m]]] . BigCGTransform[m-1] . KroneckerProduct[SchurTrans,Id];,{m,2,n}];
	SchurTrans
];
